...
/* audoramdisk.h                        */
/****************************************/
// Audio RAMdisk code - based on KOS Ramdisk.c
//   Copyright (C)2002,2003 Dan Potter

// This version is the same but uses audio RAM.
// Make sure spm_mem_xxx is initialized before using
// Also note that only the data goes in SPU RAM!
// Not all functions are tested. Not all necessarily
// work. Complex memory handling since some is CPU
// and some is SPU.

#ifdef WIN32
/* prepare the RAMdisk - not used here */	
void spu_ramdisk_init() {
}

/* Shutdown the RAMdisk - not used here */
void spu_ramdisk_shutdown() {
}
#else
#include <kos.h>
...
static rd_file_t * ramdisk_create_file(rd_dir_t * parent, const char * fn, int dir) {
	...
	f = (rd_file_t *)((char*)malloc(sizeof(rd_file_t)));
	f->name = strdup(p);
	f->size = 0;
	...
	f->usage = 0;
	if (!dir) {
		f->data = (char*)snd_mem_malloc(1024);
		f->datasize = 1024;
	} else {
		f->data = (char*)malloc(sizeof(rd_dir_t));
		f->datasize = 0;
	}
...
static ssize_t ramdisk_read(void * h, void *buf, size_t bytes) {
	...
	if (fd < MAX_RAM_FILES && fh[fd].file != NULL && !fh[fd].dir) {
		...
		spu_memread(buf, (uint32)(((uint8 *)fh[fd].file->data) + fh[fd].ptr), bytes);
...
static ssize_t ramdisk_write(void * h, const void *buf, size_t bytes) {
	...
	if (fd < MAX_RAM_FILES && fh[fd].file != NULL && !fh[fd].dir && fh[fd].file->openfor == OPENFOR_WRITE) {
		...
		if ((fh[fd].ptr + bytes) > fh[fd].file->datasize) {
			...
			/* We won't support that here - but we'll fake it */
			/* This will assume that we are writing everything in one go! Otherwise */
			/* data will be lost from the beginning of the file. */
			if (fh[fd].file->datasize==0) {
				free(fh[fd].file->data);
			} else {
				snd_mem_free((uint32)fh[fd].file->data);
			}
			// See? We don't copy the old data across!
			void * np = (void*)snd_mem_malloc((fh[fd].ptr + bytes) + 4096);
			if (np == NULL)
				goto error_out;
		...
		/* Copy out the requested amount */
		spu_memload((uint32)(((uint8 *)fh[fd].file->data) + fh[fd].ptr), (void*)buf, bytes);
...
int spu_ramdisk_attach(const char * fn, void * obj, size_t size) {
	void 		*fd;
	rd_file_t	*f;
...
int spu_ramdisk_detach(const char * fn, void ** obj, size_t * size) {
	void		*fd;
	rd_file_t	*f;
...
int spu_ramdisk_init() {
	/* Create an empty root dir */
	rootdir = (rd_dir_t *)((char*)malloc(sizeof(rd_dir_t)));
	LIST_INIT(rootdir);
	root = (rd_file_t *)((char*)malloc(sizeof(rd_file_t)));
	root->name = strdup("/");
	root->size = 0;
...
int spu_ramdisk_shutdown() {
	rd_file_t *f1, *f2;
	/* For now assume there's only the root dir, since mkdir and
	   rmdir aren't even implemented... */
	f1 = LIST_FIRST(rootdir);
	while (f1) {
//		printf("spu_ramdisk_shutdown: Freeing %s\n", f1->name);
		f2 = LIST_NEXT(f1, dirlist);
		if (f1->name) {
			free(f1->name);
//			printf("spu_ramdisk_shutdown:  Freeing name buffer.\n");
		}
		if (f1->data) {
			if (f1->datasize == 0) {
				free(f1->data);	
			} else {
				snd_mem_free((uint32)f1->data);
			}
//			printf("spu_ramdisk_shutdown:  Freeing data buffer.\n");
		}
//		printf("spu_ramdisk_shutdown:  Freeing file entry.\n");
		free(f1);
		f1 = f2;
	}

	debug("spu_ramdisk_shutdown: Freeing root dir.\n");
	free(rootdir);
	free(root);
